...
(define-non-fungible-token Keros-NFT-testnet uint)
...
;; Define variables
;; Define a map data structure for minted NFTs <token-id, token-uri>
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Keros-NFT-testnet token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? Keros-NFT-testnet token-id sender recipient)
  )
)

;; Mint a new NFT.
(define-public (mint (recipient principal) (token-uri (string-ascii 150)))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? Keros-NFT-testnet token-id recipient))

    ;; Update the last minted token ID.
    (var-set last-token-id token-id)
    ;; Save token uri into map
